...
void rMatMul3x1(double mat33[3][3], double vec31[3])
{
    for(i_comr=0; i_comr<3; i_comr++)
    {
        ...
        for(j_comr=0; j_comr<3; j_comr++)
        {
            Matout31[i_comr] = Matout31[i_comr] + mat33[i_comr][j_comr]*vec31[j_comr];
        }
    }
}
...
void rMatMul3x3(double mat331[3][3], double mat332[3][3])
{
    for(i_comr=0; i_comr<3; i_comr++)
    {
        for(j_comr=0; j_comr<3; j_comr++)
        {
            ...
            {
                Matout33[i_comr][j_comr] = Matout33[i_comr][j_comr] + mat331[i_comr][k_comr]*mat332[k_comr][j_comr];
            }
        }
    }
}
...
void rRM_to_Quat(double DCM2Q_in[3][3])
{
    ...
    if(tr > 0.0)
    {
        ...
        if(abs_f(DCM2Q_out_den) <= c_dividebyzerovalue)
        {
...
{
	double in_Quat_norm_Mag;
	if(in_Quat_norm[3] < 0.0)
	{
        in_Quat_norm[0] = -1.0 * in_Quat_norm[0];
        in_Quat_norm[1] = -1.0 * in_Quat_norm[1];
        in_Quat_norm[2] = -1.0 * in_Quat_norm[2];
        in_Quat_norm[3] = -1.0 * in_Quat_norm[3];
	}

	in_Quat_norm_Mag = sqrt((in_Quat_norm[0] * in_Quat_norm[0]) + (in_Quat_norm[1] * in_Quat_norm[1]) + (in_Quat_norm[2] * in_Quat_norm[2]) +(in_Quat_norm[3] * in_Quat_norm[3]));

    if(abs_f(in_Quat_norm_Mag) <= c_dividebyzerovalue)
    {
        in_Quat_norm_Mag = c_dividebyzerovalue;
    }

    out_Quat_norm[0] = in_Quat_norm[0] / in_Quat_norm_Mag;
    out_Quat_norm[1] = in_Quat_norm[1] / in_Quat_norm_Mag;
    out_Quat_norm[2] = in_Quat_norm[2] / in_Quat_norm_Mag;
    out_Quat_norm[3] = in_Quat_norm[3] / in_Quat_norm_Mag;
...
void rVectorNorm(double vecnorm_in[3])
{
    ...
    if(abs_f(vecnorm_mag) <= c_dividebyzerovalue)
    {
...
void rMatMul34x1(double mat34[3][4], double vec41[4])
{
    for(i_comr=0; i_comr<3; i_comr++)
    {
        ...
        {
            Matout341[i_comr] = Matout341[i_comr] + mat34[i_comr][j_comr]*vec41[j_comr];
        }
    }
}
...
void rMatMul43x1(double mat43[4][3], double vec31[4])
{
    for(i_comr=0; i_comr<4; i_comr++)
    {
        ...
        for(j_comr=0; j_comr<3; j_comr++)
        {
            Matout431[i_comr] = Matout431[i_comr] + mat43[i_comr][j_comr]*vec31[j_comr];
        }
    }
}
...
void rMatMul44x1(double mat44[4][4], double vec41[4])
{
    for (i_comr = 0; i_comr < 4; i_comr++)
    {
        ...
        for (j_comr = 0; j_comr < 4; j_comr++)
        {
          Matout441[i_comr] =  Matout441[i_comr] + mat44[i_comr][j_comr] * vec41[j_comr];
        }
    }
}
