...
void antennaCommand1(void){
    ...
    REG32(ANTENNA_STATUS_REGISTER)                       = 0x00006803;
    return;
}
void antennaCommand2(void){
	...
	REG32(ANTENNA_WRITE_DATA_ADDRESS_BASE + 0x00000008 ) = 0x000000C3;
	REG32(ANTENNA_SLAVE_ADDRESS)                         = 0x00000063;
	REG32(ANTENNA_STATUS_REGISTER)                       = 0x00004803;
    return;
}
...
SA1_status_t rHAL_SA1_Deploy_status_check()
{
	...
	{
		if((IO_IN_LATCH_REGISTER_4 & SA1_DEPLOY_ST) == SA1_DEPLOY_ST)
		{
			SA1_status = SA1_DEPLOYED;
		}
		else
		{
			if(TC_data_command_Table.SA_PanelHeater_Timeout <= (Major_Cycle_Count - SA1_Deploy_cmd_rcvd_time) )//testing for 10sec
			{
...
SA2_status_t rHAL_SA2_Deploy_status_check()
{
	...
	{
		if((IO_IN_LATCH_REGISTER_4 & SA2_DEPLOY_ST) == SA2_DEPLOY_ST)
		{
...
//			IO_LATCH_REGISTER_5 = Out_latch_5.data;
		...
		{
			if(TC_data_command_Table.SA_PanelHeater_Timeout <= (Major_Cycle_Count - SA2_Deploy_cmd_rcvd_time) )//testing for 10sec
			{
